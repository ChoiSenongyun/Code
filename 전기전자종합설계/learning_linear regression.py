...
y_train=np.zeros(case_number)
for i  in range(case_number):
        y_train[i]=normal[i]['max_voltage']
        if i%2==0:
            y_train[i] = worst_1000[i//2]['pene']
        if i%2==1:
            y_train[i] = best_1000[i // 2]['pene']
...
"""
x_test=np.zeros((len(best),x_number))
for i in range(len(best)):
    for j in range(pv_num):
        a = np.where(all_bus_name == str(int(best_bus[i][j])))
        x_test[i][a[0].tolist()[0]]=best_kva[i][j]
