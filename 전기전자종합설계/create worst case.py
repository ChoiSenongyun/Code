...
with open('worst_100.csv','w') as file :
    write = csv.writer(file)
    write.writerows(normal)
with open('worst_100_bus.csv','w') as file :
    write = csv.writer(file)
    write.writerows(normal_bus)
with open('worst_100_kva.csv','w') as file :
    write = csv.writer(file)
    write.writerows(normal_kva)
"""
with open('maxabnormal.csv','w') as file :
    write = csv.writer(file)
    write.writerows(maxabnormal)
with open('maxabnormal_bus.csv','w') as file :
    write = csv.writer(file)
    write.writerows(maxabnormal_bus)
with open('maxabnormal_kva.csv','w') as file :
    write = csv.writer(file)
    write.writerows(maxabnormal_kva)
with open('minabnormal.csv','w') as file :
    write = csv.writer(file)
    write.writerows(minabnormal)
with open('minabnormal_bus.csv','w') as file :
    write = csv.writer(file)
    write.writerows(minabnormal_bus)
with open('minabnormal_kva.csv','w') as file :
    write = csv.writer(file)
    write.writerows(minabnormal_kva)
"""
"""
for i in range(0,n):
    for j in range(0,m):
        plt.scatter(penetration_axis[i], max_vol[i][j], s=5,color='black')

plt.axhline(y=1.05,color='r')
#plt.scatter(float(select_case_1[1]),round(float(select_case_1[0]),8), s=20,color='Blue')
#plt.scatter(float(select_case_2[1]),round(float(select_case_2[0]),8), s=20,color='Blue')
plt.xlabel("Penetration levels [%] ")
plt.ylabel("Max Voltages [pu]")
plt.savefig('Max')
plt.show()
"""
"""
for i in range(0,n):
    for j in range(0,m):
        plt.scatter(penetration_axis[i], min_vol[i][j], s=5,color='black')

plt.axhline(y=0.95,color='r')
#plt.scatter(float(select_case_1[1]),round(float(select_case_1[0]),8), s=20,color='Blue')
#plt.scatter(float(select_case_2[1]),round(float(select_case_2[0]),8), s=20,color='Blue')
plt.xlabel("Penetration levels [%] ")
plt.ylabel("Min Voltages [pu]")
plt.savefig('Min')
plt.show()
"""
"""
plt.scatter(x*100, z, s=5)
